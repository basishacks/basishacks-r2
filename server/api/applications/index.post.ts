...
import { getOAuth2ApplicationCountByOwner, MAX_APPLICATIONS_PER_USER } from '~~/server/utils/database/oauth2_applications'
...
export default defineEventHandler(applyRateLimit(async (event) => {
  ...
  const canCreateFirstParty = hasPermission(user.role, DevPermissions.PORTAL_APPLICATIONS_CREATE_FIRST_PARTY)
