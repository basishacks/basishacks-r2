...
import { getOAuth2Application, addOAuth2ApplicationScopes } from '~~/server/utils/database/oauth2_applications'
...
export default defineEventHandler(applyRateLimit(async (event) => {
  ...
  const isAdmin = hasPermission(user.role, DevPermissions.PORTAL_APPLICATIONS_VIEW_ALL) ||
