...
export const teams = sqliteTable(
  ...
  (table) => [
    ...
    check(
      'teams_pathway_check',
      sql`${table.pathway} IN (NULL, 'junior', 'senior')`,
    ),
  ],
)
...
export const teamScores = sqliteTable(
  ...
  (table) => [
    primaryKey({ columns: [table.team_id, table.judge_user_id] }),
  ],
)
...
export const users = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex('idx_users_lower_email').on(
      sql`lower(${table.email})`,
    ),
    index('idx_users_team_id').on(table.team_id),
    check(
      'users_role_check',
      sql`${table.role} IN ('participant', 'judge', 'admin')`,
    ),
  ],
)
...
export const ballotScores = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex('idx_unique_ballot_project').on(
      table.ballot_id,
      table.project_id,
    ),
    check(
      'ballot_scores_score_check',
      sql`${table.score} IN (NULL, 1, 2, 3, 4, 5)`,
    ),
  ],
)
...
// ---------------------------------------------------------------------------
export const scVotes = sqliteTable('sc_votes', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const seasons = sqliteTable(
  ...
  (table) => [
    check(
      'seasons_is_active_check',
      sql`${table.is_active} IN (0, 1)`,
    ),
  ],
)
