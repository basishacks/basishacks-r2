...
export async function createBallot(event: H3Event, userID: number): Promise<Ballot> {
  return (event.context.db
    .prepare('INSERT INTO ballots(user_id) VALUES(?) RETURNING *')
    .bind(userID)
    .first() as Ballot)!;
}
...
export async function getBallotByUser(event: H3Event, userID: number): Promise<Ballot | null> {
  return event.context.db
    .prepare('SELECT * FROM ballots WHERE user_id = ?')
    .bind(userID)
...
export async function updateBallot(event: H3Event, ballot: Ballot): Promise<void> {
  event.context.db
    .prepare('UPDATE ballots SET reasoning = ?, submitted = ? WHERE id = ?')
...
export async function createBallotScore(
  ...
  return (event.context.db
    .prepare('INSERT INTO ballot_scores(ballot_id, project_id) VALUES(?, ?) RETURNING *')
...
export async function getBallotScores(event: H3Event, ballotID: number): Promise<BallotScore[]> {
  return (
    event.context.db
      .prepare('SELECT * FROM ballot_scores WHERE ballot_id = ?')
      .bind(ballotID)
...
export async function getBallotScoresByTeamID(
  ...
  return (
    event.context.db
      .prepare('SELECT * FROM ballot_scores WHERE project_id = ?')
      .bind(teamID)
      .all() as { results: BallotScore[] }
  ).results;
}
...
export async function updateBallotScore(event: H3Event, score: BallotScore) {
  event.context.db
    .prepare('UPDATE ballot_scores SET score = ? WHERE id = ?')
