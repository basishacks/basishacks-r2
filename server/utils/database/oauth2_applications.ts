...
export async function getOAuth2ApplicationCountByOwner(
  ...
  const result = event.context.db
    .prepare('SELECT COUNT(*) as count FROM oauth2_applications WHERE owner_id = ?')
    .bind(ownerId)
...
export async function createOAuth2Application(
  ...
  event.context.db
    ...
    .bind(client_id, '', name, description, proxyMicrosoft ? 1 : 0, type, ownerId)
    .run();
...
export async function getOAuth2Application(
  ...
  const result = event.context.db
    .prepare(`SELECT * FROM oauth2_applications WHERE client_id = ?`)
    .bind(clientID);
...
export async function getAllOAuth2Applications(event: H3Event): Promise<OAuth2Application[]> {
  return (
    event.context.db.prepare('SELECT * FROM oauth2_applications ORDER BY name ASC').all() as {
...
export async function deleteOAuth2Applications(event: H3Event, clientIDs: string[]) {
  for (const id of clientIDs) {
    event.context.db.prepare('DELETE FROM oauth2_applications WHERE client_id = ?').bind(id).run();
  }
}
...
export async function removeOAuth2ApplicationRedirectUri(
  ...
  event.context.db
    .prepare(`UPDATE oauth2_applications SET redirect_uris = ? WHERE client_id = ?`)
    .bind(newValue.join(' ') || null, clientID)
    .run();
}
...
export async function addOAuth2ApplicationScopes(
  ...
  event.context.db
    .prepare(`UPDATE oauth2_applications SET permissions = ? WHERE client_id = ?`)
    .bind(combined.join(' ') || null, clientID)
    .run();
}
