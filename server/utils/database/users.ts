...
export async function updateUserProfileTheme(event: H3Event, user: User) {
  const result = event.context.db
    .prepare('UPDATE users SET profile_theme = ? WHERE id = ?')
...
export async function updateUserProfilePicture(event: H3Event, user: User) {
  const result = event.context.db
    .prepare('UPDATE users SET profile_picture = ? WHERE id = ?')
...
export async function deleteUsers(event: H3Event, userIDs: number[]) {
  for (const id of userIDs) {
    ...
    event.context.db.prepare('DELETE FROM team_scores WHERE judge_user_id = ?').bind(id).run();

    event.context.db.prepare('DELETE FROM ballots WHERE user_id = ?').bind(id).run();

    event.context.db.prepare('DELETE FROM users WHERE id = ?').bind(id).run();
  }
}
