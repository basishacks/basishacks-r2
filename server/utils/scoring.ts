...
export async function computeScores(event: H3Event, seasonID: number) {
    ...
    for (const score of scores) {
       ...
       const standing : {originality: number, presentation: number, technicality: number, theme: number, impact: number} = JSON.parse(score.scores);
    ...
    teams_junior.sort((a, b) => {return b.score! - a.score!})
    teams_senior.sort((a, b) => {return b.score! - a.score!})
