...
export async function validateOAuth2AuthorizationRequest(
    ...
    try {
        requestedScopes = decodeURI(scope)
            .split(" ")
            .filter((s) => s);
    } catch {
        throw createError({
...
export function usedSensitiveScopes(session: AuthorizeSession): boolean {
    for (const scope of session.scopes) {
        ...
        if (s?.sensitive) {
            console.log(
                "[Authorization -> OAuth2] Application " +
                    session.application.client_id +
                    " requests sensitive scope " +
                    scope +
                    ", triggering consent",
            );
            return true;
...
export function determinePostMicrosoft(event: any, session: AuthorizeSession): string {
    ...
    if (sensitive) {
        ...
        return (
            "/api/oauth2/authorize?client_id=" +
            session.application.client_id +
            "&scope=" +
            session.scopes.join(" ") +
            "&redirect_uri=" +
            ...
            session.bh_state +
            "&response_type=code" +
            (session.bh_verifier_challenge
                ? "&code_challenge=" + session.bh_verifier_challenge
                : "") +
            (session.bh_verifier_challenge_method
                ? "&code_challenge_method=" + session.bh_verifier_challenge_method
                : "")
        );
    }
