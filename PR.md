...
**159 files changed**: +19,486 insertions, −1,944 deletions across application code, server logic, middleware, shared schemas, documentation, and tests.

## Code of Conduct Overhaul (rules.vue)

The `app/pages/rules.vue` page was completely rewritten (**506 lines added**, 53 removed) from a minimal stub into a comprehensive Code of Conduct and rules document:

- **Essential Requirements at a Glance** — Full source code requirement, clean README mandate, public demo/repo URLs, hardware provision rules
- **Detailed Rules & Requirements** — Expanded into 10+ sections covering project submission requirements, team composition rules, judging criteria, scoring methodology, disqualification grounds
- **Code of Conduct** — Ethical guidelines, professional standards, anti-harassment policy, reporting procedures
...
- **better-sqlite3**: `^12.11.1` → `^13.0.2` (major)
- **openai**: `^6.49.0` → `^7.1.0` (major) - requires Node.js 22+
- **vue-router**: `^4.6.4` → `^5.2.0` (major)
- **typescript**: `6.0.3` → `^6.0.3` (unpinned, but no v7)
- Overrides changed from exact pins to semver ranges (`^8.0.16`, `^7.0.1`)

---

## Additional Security Hardening (Round 2)

### Open Redirect Prevention

- **`server/api/login.get.ts`**: `constructOnSiteLoginURL()` now validates `post_login_redirect` — blocks absolute URLs (`http://`, `https://`, `//`), only allows relative paths. Prevents phishing via redirect parameter.
- **`server/api/oauth2/dccallback.get.ts`**: Redirect parameter validated against absolute URLs; only relative redirects allowed. Removed `token` and `token_raw` from session store — JWT no longer stored in session cookie data.

### Unified Client Credential Errors

- **`server/api/oauth2/token.post.ts`**: Merged separate `"Invalid client_id"` and `"Invalid client_secret"` errors into unified `"Invalid client credentials"` — prevents client ID enumeration via error message differentiation.

### PKCE Hardening (S256 Only)

- **`server/utils/oauth2-validate.ts`**: Removed support for `"plain"` code_challenge_method (RFC 7636 §4.4.2 — provides no additional security). Now **requires S256** exclusively. Error message updated from `"must be S256 or plain"` to `"must be S256"`.

### Audit Logging for Admin Impersonation

- **`server/api/auth/impersonate.post.ts`**: Added `[AUDIT]` prefix console log recording admin ID, email, target user ID, and target email. Changed 404 to 403 when target user doesn't exist to prevent user enumeration.

### Startup Environment Validation

- **`server/plugins/validate-environment.ts`**: Added `NUXT_OAUTH2_JWT_SECRET` validation — fatal error and process exit if shorter than 32 bytes. Existing `NUXT_SESSION_PASSWORD` validation strengthened (fatal in production, warning in dev).

### Dependency Updates

- **openai**: ^6.46.0 → ^6.49.0
- **bun.lock**: Updated lockfile for all transitive dependencies
...
## Test Suite Expansion (more than doubled)
...
**Test count:** Over 2x the original suite. See [testing docs](/guide/testing) for current live count.

- Main branch: 639 tests across 62 files
- This branch: dynamically tracked (see `tests/.test-meta.json`)
- New test files added: 25+
- All tests pass with zero failures
- **100% coverage** across statements, branches, functions, and lines on all tracked source files
...
## ⚠️ Security Logic Changes & Breaking Changes

### 1. Input validation hardened (`shared/schemas.ts`)

**What changed:** All Zod schemas now enforce explicit `min()`/`max()` length bounds. Previously many fields had no maximum length. Select schemas were also exported publicly. Key MAX constant adjustments:

- `MAX_PROJECT_DESCRIPTION_LENGTH`: 2000 → **2147483647** (Integer.MAX_VALUE)
- `MAX_PROJECT_SOURCE_LENGTH`: 2000 → **2147483647**
- `MAX_REASONING_LENGTH`: 2000 → **2147483647**
- `MAX_USER_NAME_LENGTH`: 30 → **50**
- `MAX_URL_LENGTH`: 2048 (unchanged) All error messages updated to generic "is too long" instead of hardcoded number.

**Breaking?** YES — existing data exceeding new max lengths will be rejected on write. If your database contains records with fields longer than the new limits (e.g. `MAX_USER_NAME_LENGTH=50`), API PATCH/POST requests will now return validation errors. The description/reasoning increases to Integer.MAX_VALUE mean virtually no limit — projects with very long descriptions will now be accepted.

**Action:** Truncate or clean any stored user names longer than 50 characters before writing back through the API. No action needed for descriptions or reasoning — the limits were raised.

**Status: ✅ Applied.** The MAX constants were updated in `shared/schemas.ts`, exported for frontend use, and corresponding `:maxlength` attributes added to all form inputs (`ProjectForm.vue`, `JudgingCard.vue`, `voting.vue`).

### 2. PKCE hardening — S256 only, plain removed (`server/utils/oauth2-validate.ts`)

**What changed:** `code_challenge_method=plain` is no longer accepted. Only `S256` is allowed (RFC 7636 §4.4.2). Error message changed from `"must be S256 or plain"` to `"must be S256"`.

**Breaking?** YES — any OAuth2 client using `code_challenge_method=plain` will now receive a 400 error. This is a deliberate security fix — plain PKCE provides no additional security.

**Action:** Update all OAuth2 client code to use `code_challenge_method=S256` with a SHA-256 code challenge.

**Status: ✅ Applied.** The server now rejects `plain` and only accepts `S256`.

### 3. Redirect URI validation always enforced (`server/utils/oauth2-validate.ts`)

**What changed:** Previously `redirect_uri` was only validated "if provided". Now it's **always validated** against the application's registered redirect URIs, and a missing `redirect_uri` parameter causes an error. Also, `redirect_uri` is now required in the validation function (parameter is no longer optional).

**Breaking?** YES — OAuth2 clients that omit the `redirect_uri` parameter entirely will now get a 400 error. All authorization requests must include a registered redirect URI.

**Action:** Ensure all OAuth2 authorization requests include a `redirect_uri` that exactly matches a registered URI for the application.

**Status: ✅ Applied.** The server always validates redirect_uri against registered URIs.

### 4. managed to only allow https and http://localhost (`shared/schemas.ts`)

**What changed:** `ManageRedirectUriRequest` now enforces `https://` or `http://localhost` only (via `.refine()` check). Plain `http://` URLs to non-localhost hosts are rejected.

**Breaking?** YES — any application with a redirect URI registered as `http://example.com/callback` (non-https, non-localhost) will be rejected.

**Action:** Use `https://` for all production redirect URIs, or `http://localhost` for local development.

**Status: ✅ Applied.** The Zod schema enforces https:// or http://localhost.

### 5. `getValidatedRouterParams` replaces manual `parseInt` (multiple API files)

**What changed:** All API route handlers switched from `parseInt(getRouterParam(event, "id")!)` to `getValidatedRouterParams(event, Schema.parse)` using Zod route param schemas (`PositiveIntParam`, `ApplicationIdParams`, etc.).This adds type coercion, positive-integer validation, and proper error responses for invalid params.

**Breaking?** NO — behavior is identical for valid positive integer inputs. Invalid params (NaN, negative, zero, non-numeric strings) now produce clean Zod validation errors rather than `NaN` propagating through the system.

**Action:** No action needed for valid inputs. Non-numeric route params will now return 400 instead of potentially causing unexpected behavior.

**Status: ✅ Applied.** All API handlers use `getValidatedRouterParams`.

### 6. Rate limiting defaults changed (60 → 6000 req/min)

**What changed:** The default rate limit increased from 60 to 6000 requests per minute. Rate limits are now configurable via environment variables (`RATE_LIMIT_GENERAL_MAX`, `RATE_LIMIT_AUTH_MAX`, `RATE_LIMIT_VOTE_MAX`, `RATE_LIMIT_UPLOAD_MAX`). Multiple endpoint-specific presets were added (`AUTH`, `VOTE`, `UPLOAD`). Many previously-unlimited endpoints (team CRUD, submission, user management, OAuth2 token/scopes/secrets) now have rate limiting applied.

**Breaking?** PARTIAL — endpoints that were previously unlimited now have rate limits. The 6000/min default is generous enough to not affect normal usage, but automated scripts or misconfigured clients may hit the limit. The increased default (from 60 → 6000) reduces false positives for the general API.

**Action:** If you have automated scripts hitting these endpoints, they will now be subject to rate limiting. Adjust script pacing if you see 429 responses.

### 7. Token/client validation error messages unified (`server/api/oauth2/token.post.ts`)

**What changed:** Previously, distinct error messages `"Invalid client_id"` vs `"Invalid client_secret"` allowed client ID enumeration. Now both cases return `"Invalid client credentials"`.

**Breaking?** YES — if your monitoring or logging relies on distinguishing these two error states, it will break. This is a deliberate security hardening to prevent user enumeration.

**Action:** Update any monitoring/logging that checks for the old `"Invalid client_id"` or `"Invalid client_secret"` messages. Use the unified `"Invalid client credentials"` instead, or log the submitted `client_id` separately for debugging.

**Status: ✅ Applied.** Token endpoint returns unified `"Invalid client credentials"`.

### 8. Open redirect prevention (`server/api/login.get.ts`, `server/api/oauth2/dccallback.get.ts`)

**What changed:** `login.get.ts` now blocks absolute URLs and protocol-relative URLs in `post_login_redirect` — only relative paths are accepted. `dccallback.get.ts` now blocks absolute URLs in the `redirect` query parameter, redirecting to `/dashboard` instead.

**Breaking?** YES — any existing integration that passes an absolute URL to the redirect parameter (e.g., `?redirect=https://app.example.com/callback`) will silently be redirected to `/dashboard` instead. Only relative paths like `?redirect=/dashboard/teams` work now.

**Action:** Update any external links that pass absolute URLs to redirect parameters. Use relative paths only.

**Status: ✅ Applied.** Both `login.get.ts` and `dccallback.get.ts` block absolute redirects.

### 9. Impersonation error code changed (404 → 403, `server/api/auth/impersonate.post.ts`)

**What changed:** When impersonating a non-existent user, the error code changed from 404 to 403, with a new message `"Cannot impersonate this user"`. Audit logging added.

**Breaking?** YES — any integration relying on a 404 response to detect non-existent users will no longer work. 403 is returned instead to prevent user enumeration.

**Action:** Update any monitoring that checks for 404 on the impersonate endpoint. The endpoint now returns 403 for both insufficient permissions and non-existent targets.

**Status: ✅ Applied.** Impersonation returns 403 with audit logging.

### 10. Microsoft OAuth2 redirect URI construction changed (`server/utils/oauth2.ts`)

**What changed:** `oAuth2Config.redirectUri` changed from a static string to a getter function. `structureLink()` now uses `buildMicrosoftRedirectUri()` which constructs the full URL from origin + path, instead of concatenating a base URL with the path. Lazy evaluation ensures the env variable is read at call time, not at module import time.

**Breaking?** PARTIAL — if your `.env` sets `CURRENT_URL_ORIGIN` or `MICROSOFT_REDIRECT_URI`, ensure they are correct. The new function always builds the redirect URI using `new URL(path, base).href` which normalizes trailing slashes and path structure.

**Action:** Verify `CURRENT_URL_ORIGIN` and `MICROSOFT_REDIRECT_URI` in your `.env` file produce the correct full redirect URI. The base origin should not have a trailing slash.

**Status: ✅ Applied.** `structureLink()` uses `buildMicrosoftRedirectUri()` with proper URL constructor.

### 11. User creation flow changed — email-code login removed (`server/utils/database/users.ts`)

**What changed:** The `addCodeToUser()` function was removed and replaced with `createUserFromMicrosoftProfile()`. The email-verification-code login flow (which sent 6-digit codes) was removed entirely. The `login_code` and `login_expiry` columns were removed from the `users` table (via `schema.ts` diff).

**Breaking?** YES — any existing code or integrations that used the email-code login flow will no longer work. The only login method is now Microsoft OAuth2. Users in the database that were created through the email-code flow remain, but they can only authenticate via Microsoft now.

**Action:** Remove any code that calls `addCodeToUser` or references `login_code`/`login_expiry`. All new user creation happens through Microsoft OAuth2.

**Status: ✅ Applied.** `addCodeToUser` removed, `createUserFromMicrosoftProfile` added.

### 12. Data type narrowing in awards helpers (`server/utils/database/awards.ts`)

**What changed:** `parseMeta()` and `resolveAward()` changed their parameter types from `string | null` to `string`. This affects internal typing only — the functions that call these always pass non-null values.

**Breaking?** NO — runtime behavior is unchanged. Strict TypeScript users may see new type errors if they call these functions directly with nullable arguments.

### 13. `bin/vite` and `entities` overrides relaxed (exact → semver range)

**What changed:** `"vite": "8.0.16"` → `"^8.0.16"` and `"entities": "7.0.1"` → `"^7.0.1"`.

**Breaking?** NO — still prevents major version breaks (vite 9, entities 8) while allowing minor/patch updates. Lockfile will resolve to the latest compatible version.

---

## XSS Audit

A comprehensive XSS audit was performed across all frontend components (`app/` directory, 50+ `.vue`/`.ts` files):

- **No `v-html` usage** — zero instances found. All user-controlled text renders through Vue's auto-escaping `{{ }}` interpolation.
- **No `innerHTML` or `dangerouslySetInnerHTML`** — zero instances found.
- **All URL outputs validated** — `SafeLink.vue` validates `href` via `isSafeUrl()`. Project URLs (demo, repo) use `safeUrl()` computed properties. User avatar URLs constructed server-side with sanitized filenames.
- **SafeComark for descriptions** — User project descriptions render through `SafeComark.vue` which escapes HTML before rendering Markdown.
- **Debug file URLs** constructed as `/assets/<uuid>` with sanitized basenames — no user-controlled filenames reach the frontend.
- **Email data** in chat URLs properly `encodeURIComponent`-escaped.

**Result: No XSS vectors found across the entire frontend.**

## Non-breaking security hardening (additive only)

These changes introduce new security features without altering existing behavior:

- **`server/middleware/security-headers.ts`** — New middleware adding 6 HTTP security headers (HSTS, CSP, etc.) to all responses. Headers only constrain browser behavior; existing API clients are unaffected.
- **`server/middleware/debug-lockdown.ts`** — New middleware that blocks `/api/debug/*` when `DISABLE_DEBUG_ROUTES` is set. Inactive by default.
- **`server/plugins/validate-environment.ts`** — New plugin that validates `NUXT_SESSION_PASSWORD` (≥32 bytes) and `NUXT_OAUTH2_JWT_SECRET` (≥32 bytes) at startup. In production, the server exits with a fatal error if either is too short or missing. **Action:** ensure both env vars are set to a value ≥32 bytes before deploying to production.
- **`server/middleware/oauth2-authorize.ts`** — Added `httpOnly: true` to error cookie. Cookie hardening only.
- **`server/utils/assets.ts`** — Switched to POSIX `basename`, simplified path prefix check. Path traversal hardening only.
- **`server/utils/oauth2-jwt.ts`** — JWT secret fetched before try block to preserve config errors. Error reporting improvement only.
- **`server/utils/url-validation.ts`** — Redirect overflow refactored (`break` instead of early `return`). Behavior unchanged.
- **`server/utils/rateLimit.ts`** — Default increased from 60→6000 req/min. New env-var-configurable presets. Backward compatible.
- **`shared/schemas.ts` exports** — Internal schemas exported for testing. No runtime impact.

### 1. `better-sqlite3` ^12 → ^13 (major)

The native SQLite addon was upgraded from v12 to v13. Users pulling this branch with an existing `node_modules/` **must delete `node_modules` and reinstall**, otherwise the stale v12 binary will fail to load:
...
The `bun install` step will automatically compile the v13 native binary via `node-gyp`. Requires a C++ build toolchain (Visual Studio Build Tools on Windows, Xcode Command Line Tools on macOS, `build-essential` on Linux).

### 2. `vue-router` ^4 → ^5 (major)

Vue Router was upgraded from v4 to v5. This is consumed by Nuxt internally. Ensure your Nuxt version is >= 4.5.1 (already satisfied by `nuxt: ^4.5.1` in `package.json`). If you were importing Vue Router APIs directly, check the [vue-router v5 migration guide](https://github.com/vuejs/router) for any API changes.

### 3. `openai` ^6 → ^7 (major)

The OpenAI SDK was upgraded from v6 to v7. The only documented breaking change is **Node.js >= 22 required**. This project already requires Node.js >= v24, so no action needed.

### 4. `typescript` ^5 → ^6 (major)

TypeScript was upgraded from v5.9 to v6.0. Review the [TypeScript 6 migration guide](https://devblogs.microsoft.com/typescript/announcing-typescript-6-0/) for new strictness defaults. Run `bun run build` and check for new type errors after upgrading. The `tsconfig.json` may need adjustments for any newly-disallowed patterns.

### 5. New dependencies: `comark` and `@comark/nuxt`

These packages were added for Markdown rendering. They will be installed automatically by `bun install`.

### 6. Overrides changed (exact pins → semver ranges)

The `overrides` in `package.json` were changed from exact pinned versions (`"8.0.16"`, `"7.0.1"`) to semver ranges (`"^8.0.16"`, `"^7.0.1"`). This allows minor/patch updates while preventing major breaks. If you need exact reproducibility, add a lockfile or use `bun install --frozen-lockfile`.

### 7. PR.md removed from git tracking

`PR.md` is no longer tracked by git. It remains on disk as an untracked file. To stop seeing it in `git status`, add it to `.git/info/exclude`.
...
551513c docs(shared): update schemas doc with all length constants; update index with test count
10a615d docs(frontend): add SafeLink and SafeComark component docs
c10c391 docs(backend): update API ref and server utils with security features
1a80915 docs: update OAuth2 and middleware docs with S256-only PKCE and env validation
82c35fa docs(arch): update overview and auth docs with security hardening details
281d61d docs(guide): update getting-started and project-overview with test count and security
6ef56ce docs: update performance config and testing guide with current test count
2102182 docs(deployment): update security and rate-limiting docs with latest hardening
e26ebc6 chore(deps): update openai to ^6.49.0
4087836 test: align with security hardening - S256-only PKCE, 403 for unknown impersonation target
5b7c412 fix(security): enforce S256 PKCE only, validate JWT secret at startup
3136aef fix(oauth2): block open redirect in dccallback, unify client error messages
68447b2 fix(auth): prevent open redirect in login, add impersonation audit log
250f6e0 docs: add comprehensive PR.md with full state of security hardening and test expansion
...
37111cc feat(docs): dynamic test count via TestCount component + test meta data loader
3831cfe chore(deps): openai ^6.49.0 -> ^7.1.0, vue-router ^4.6.4 -> ^5.2.0
33baf36 chore(deps): better-sqlite3 ^12.11.1 -> ^13.0.2
0909ffd chore(deps): unpin overrides to semver ranges, typescript to ^6, run bun update
c8116e1 chore(deps): update lockfile
```
