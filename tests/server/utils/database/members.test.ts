...
describe('members database helpers', () => {
  ...
  beforeEach(() => {
    ...
    event.context.db.prepare(
      "INSERT INTO users(email, name, team_id) VALUES('bob@example.com', 'Bob', 1)",
    ).run()
    event.context.db.prepare(
  ...
  describe('getAllTeamMembers', () => {
    it('includes past team members alongside current members', async () => {
      ...
      event.context.db.prepare(
        'INSERT INTO user_past_teams(user_id, team_id) VALUES(3, 1)',
      ).run()
  ...
  describe('getUserPastTeams', () => {
    it('returns past teams for a user', async () => {
      event.context.db.prepare(
        'INSERT INTO user_past_teams(user_id, team_id) VALUES(1, 2)',
      ).run()
