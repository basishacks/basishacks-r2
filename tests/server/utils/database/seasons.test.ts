...
describe('seasons database helpers', () => {
  ...
  describe('getSeasons', () => {
    it('returns all seasons ordered by id', async () => {
      event.context.db.prepare(
        "INSERT INTO seasons(name, is_active) VALUES('Season 1', 0)",
      ).run()
      event.context.db.prepare(
        "INSERT INTO seasons(name, is_active) VALUES('Season 2', 1)",
      ).run()
  ...
  describe('getActiveSeason', () => {
    it('returns the active season when one exists', async () => {
      event.context.db.prepare(
        "INSERT INTO seasons(name, is_active) VALUES('Inactive', 0)",
      ).run()
      event.context.db.prepare(
