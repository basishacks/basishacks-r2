...
describe('GET /api/ballot', () => {
  it('returns 409 when hackathon is not in voting phase', async () => {
    vi.mocked(globalThis.requireUser).mockResolvedValue({ id: 1, team_id: 1, role: 'participant' })
  ...
  it('returns 403 when user has no team', async () => {
    ...
    vi.mocked(globalThis.requireUser).mockResolvedValue({ id: 1, team_id: null, role: 'participant' })
  ...
  it('returns eligible projects and scores for a voter', async () => {
    ...
    const userTeam = seedTeam(ctx, { name: 'My Team', project_submitted: 1, pathway: 'junior', season_id: season.id })
    seedUser(ctx, { email: 'voter@basischina.com', team_id: userTeam.id })

    // Create another submitted team in the same pathway
    seedTeam(ctx, { name: 'Other Team', project_submitted: 1, pathway: 'junior', season_id: season.id })
