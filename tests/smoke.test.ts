...
describe('in-memory SQLite database', () => {
  ...
  it('supports INSERT and SELECT', () => {
    db.prepare('INSERT INTO hackathon (id, status, start_timestamp, end_timestamp, voting_start_timestamp, voting_end_timestamp, results_open_timestamp) VALUES (1, ?, 0, 0, 0, 0, 0)')
      .bind('not_started')
      .run()
  ...
  it('foreign keys are enforced', () => {
    ...
    expect(() => {
      db.prepare("INSERT INTO users (email, role, team_id) VALUES ('test@test.com', 'participant', 9999)").run()
    }).toThrow()
  })
})