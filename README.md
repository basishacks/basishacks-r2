...
| Layer           | Technology                                                                  |
| --------------- | --------------------------------------------------------------------------- |
| Framework       | Nuxt 4 (^4.5.1)                                                             |
| UI              | @nuxt/ui ^4.10.0 (Tailwind CSS v4)                                          |
| Language        | TypeScript ^6.0.3                                                            |
| Runtime         | Node.js >= v24 or Bun (dual-runtime support)                                |
| Package Manager | Bun (preferred); npm works                                                  |
| Database        | SQLite via Drizzle ORM (bun:sqlite under Bun, better-sqlite3 ^13.0.2)        |
| Auth            | nuxt-auth-utils 0.5.29 (session-based)                                      |
| JWT             | jose ^6.2.5 (OAuth2 access tokens)                                          |
| Validation      | Zod 4.x (^4.4.3)                                                            |
| Fonts           | @nuxt/fonts ^0.14.0 (local provider)                                        |
| Icons           | @iconify-json/lucide, @iconify-json/material-symbols, @iconify-json/simple-icons |
| Linting         | @nuxt/eslint 1.16.0 + Prettier ^3.9.6                                       |
| AI              | openai ^7.1.0 (DeepSeek API integration)                                    |
| Deployment      | Node.js server (VPS); Nitro node-server preset                              |
...
>
> Overrides use semver ranges (not exact pins) to prevent major breaks while allowing minor/patch updates:
> - `vite`: `^8.0.16` — Vite `8.1.x` combined with Nuxt creates HMR conflicts
> - `entities`: `^7.0.1` — `entities@8` breaks Vue's SFC compiler`
...
Tests are run with **Vitest** (87 test files, ~1900+ tests):

```bash
bun run test            # one-shot run (also generates tests/.test-meta.json)
bun run test:watch      # watch mode
bun run test:coverage   # with coverage
bun run test:meta       # regenerate tests/.test-meta.json only
```

The suite maintains **100% line, branch, function, and statement coverage** on all tracked source files. The canonical test count is stored in `tests/.test-meta.json` (auto-generated by `bun run test`) and displayed dynamically in the [VitePress documentation site](/guide/testing) via `<TestCount />`.

Coverage configuration in `vitest.config.ts` excludes build artifacts, generated migrations, documentation, configuration files, and test helpers.
...
### Production security & performance defaults

The following settings are configured in `nuxt.config.ts` and apply automatically to production builds:
...
- **Font display swap**: `@nuxt/fonts` is configured with `defaults.display: "swap"` and `preload: true`.
- **Request body size limit**: 10 MB max for uploads.
- **Build target**: `vite.build.target` at `es2020`.

### Security middleware (applied to all routes)

Six HTTP security headers are applied by `server/middleware/security-headers.ts`:
- `Strict-Transport-Security` (2-year HSTS with preload)
- `X-Frame-Options: DENY` (clickjacking prevention)
- `X-Content-Type-Options: nosniff`
- `Referrer-Policy: strict-origin-when-cross-origin`
- `Permissions-Policy` (disables unused features)
- `Content-Security-Policy` (10 directives restricting script/style/font/img/connect/object/base/form-action/frame-ancestors)

Debug routes (`/api/debug/*`) are blocked in production via `DISABLE_DEBUG_ROUTES`.

### OAuth2 security

- PKCE enforcement (S256 only; `plain` method removed per RFC 7636 §4.4.2)
- Authorization code single-use guarantee (RFC 6749 §4.1.2)
- Unified client credential errors (no user enumeration)
- Open redirect prevention in login and callback flows
- JWT secret validation at startup (≥ 32 bytes, fatal on failure)
- All OAuth2 endpoints rate-limited (default 600 requests/minute)
