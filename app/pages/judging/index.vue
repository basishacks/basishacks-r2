...
const filteredPast = computed(
    () => summary.value?.past.filter((item) => item.scored_count > 0) ?? [],
);
...
<template>
    <div class="mt-4">
        ...
        <p>
            ...
            <UBadge variant="outline" color="primary">Junior</UBadge>
            division,
            <span class="bold">18</span>
            participated in the
            <UBadge variant="outline" color="warning">Senior</UBadge>
            division.
        </p>
    </div>
    ...
    <div
        v-if="filteredPast && filteredPast.length > 0"
        class="my-4"
    ...
    <UAlert color="neutral" icon="i-lucide-info" class="my-4" variant="subtle">
        <template #title>
            <span class="font-bold">Regarding Past Evaluations</span>
        </template>
        <template #description>
            <p>
                If you participated in any past evaluations, they will be available here once they
                are migrated.
            </p>
        </template>
    </UAlert>
