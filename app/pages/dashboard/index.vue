...
const {
    ...
    error: teamError,
    refresh: teamRefresh,
} = await useFetch<GetTeamMembersResponse>(() => `/api/teams/${data.value?.team?.id}/users`, {
    dedupe: "defer",
});
if (teamError.value) {
        ...
        <template v-else>
            <p class="mb-4">
                ...
                <template v-else-if="hackathon?.status === 'paused'">
                    The hackathon is paused as the organizers figure out the secrets of the
                    universe... Please check back later!
                </template>
            </p>
            <p class="mb-4">
                Please review the
            ...
            <UPageGrid>
                <UPageCard v-for="(card, index) in actions" :key="index" v-bind="card" />
            </UPageGrid>
        </template>
    </div>
</template>
