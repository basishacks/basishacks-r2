...
async function onSubmit(event: FormSubmitEvent<SubmitVoteRequest>) {
  if (!confirm('Are you sure you want to submit the vote? You cannot edit them after submission.'))
    return;
...
<template>
  <div class="mt-4">
    ...
    <p class="mb-4">
      You will see four projects below. Please distribute 12 stars among them. Each project must
      receive 1-5 stars, and you must use all of them. Please also write a short paragraph
      justifying your vote. Make sure you include details about ALL four projects!
    </p>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      <VotingProjectCard v-for="project in data?.projects" :key="project.id" :project="project" />
    </div>

    <h2 class="text-2xl mb-4 bold">Submit vote</h2>

    <UForm :disabled="!!data?.scores" :state="state" :schema="SubmitVoteRequest" @submit="onSubmit">
      <UCard variant="subtle">
        <p v-if="!!data?.scores" class="mb-4 bold">You have voted already. Thank you!</p>
        ...
        <UFormField name="scores">
          <div class="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-2 items-center mb-4">
