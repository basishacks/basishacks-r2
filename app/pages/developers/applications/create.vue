    ...
    <template #body>
      ...
      >
        <UFormField name="name">
          <template #label>
            Application Name
            <UIcon name="i-lucide-asterisk" class="text-red-400" />
          </template>
          <UInput
            v-model="state.name"
            :maxlength="64"
        ...
        <p class="text-xs text-muted">
          First-party applications are trusted internal applications. Third-party applications are
          external integrations. Admin consent is required to create a first-party application.
        </p>
        ...
        <p class="text-xs text-muted">
          Selecting "Yes" will tell the OAuth login flow to automatically redirect the user login to
          Microsoft, instead of giving them an option to choose whether to log in with a teams code
          or with Microsoft. This option is great for creating applications that does not want to
          affiliate with basishacks accounts.
        </p>

        <UButton type="submit" :disabled="!authorized">Create Application</UButton>

        <p class="text-xs text-muted">
          API keys, secrets, and permissions can be configured after the creation of this app.
        </p>
